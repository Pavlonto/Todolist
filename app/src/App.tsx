...
function App() {
    ...
    let mappedTodolist = todolists.map(tl => {
        ...
        return <Grid item><Paper style={ {padding: "10px"} }><Todolist key={tl.id} id={tl.id} title={tl.title} tasks={taskForTodolist}
    ...
    return (
        <div className="App">
            ...
            <Container fixed>
                <Grid style={ {padding: "20px"} } container><AddItemForm addItem={addTodolist}/></Grid>
                <Grid container spacing={3}>{mappedTodolist}</Grid>
            </Container>
        </div>
    )
}
